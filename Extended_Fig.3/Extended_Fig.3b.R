...
clinical_T4<-subset(clinical,Time=='4')

#AtT1(Pretreatment)
clinical_T1<-clinical_T1[order(clinical_T1$Group2),]
patient<-c('P10','P11','P12','P2','P7','P8','P17','P19','P22','P23','P25','P29','P36')
clinical_T1<-subset(clinical_T1,Patient_ID  %in% patient)
...
patient<-c('P10','P11','P12','P2','P7','P8','P17','P19','P22','P23','P25','P29','P36')
clinical_T2<-subset(clinical_T2,Patient_ID  %in% patient)
...
mat$gene<-factor(mat$gene,levels=patient_order)
mat<-mat[order(mat$gene),]
gene_name<-c('TP53','APC','FOXO1','AR','CARD11','CHEK1','FBXW7','KMT2C','PTPRT','ALK','ARID1A','ATRX','BRAF','CCND3','CDK12','EPHA5','FANCD2','FAS','FAT1','FOXL2','GLI1','GRIN2A','IRS2','MTOR','NOTCH1','NTRK2','NUP93','PARK2','PGR','PIK3CA','PIK3CG','PREX2','PTPRS','RASA1','ROS1','SMAD3','SMAD4','SOX17','SOX9','TERT','TSC1')
...
patient<-c('P10','P11','P12','P2','P7','P8','P17','P19','P22','P23','P25','P29','P36')
clinical_T3<-subset(clinical_T3,Patient_ID  %in% patient)
...
#AtT43(PD)
clinical_T4<-clinical_T4[order(clinical_T4$Group2),]
patient<-c('P10','P11','P12','P2','P7','P8','P17','P19','P22','P23','P25','P29','P36')
clinical_T4<-subset(clinical_T4,Patient_ID  %in% patient)
patient_order<-clinical_T4$Sample_ID
library(ComplexHeatmap)
orr.assign.1 <- setNames(c("#2196F3","#FFC107","#F44336"),c('PR','SD','PD'))
clini<-HeatmapAnnotation(ORR=clinical_T4$Group2,col=list(ORR=orr.assign.1))
mat<-read.csv('snv.landscape.txt',sep='\t',header=TRUE)
library(tidyr)
library(dplyr)
mat<-subset(mat,gene  %in% patient_order)
...
pdf('landscape_snv_T4.pdf',width=6,height=8)
oncoPrint(mat,alter_fun = alter_fun,col = col,column_order=patient_order,row_order=gene_name,remove_empty_columns = FALSE, remove_empty_rows = FALSE,row_names_side="left",pct_gp = gpar(fontsize = 7),row_names_gp = gpar(fontsize = 8),column_names_gp=gpar(fontsize = 7),pct_side="right",show_column_names=TRUE,bottom_annotation = clini)
dev.off()
